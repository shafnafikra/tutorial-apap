...
public class PenjagaRestController {
    ...
    private ResponseEntity deletePenjaga(@PathVariable("noPenjaga") Long noPenjaga){
        try{
            ...
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Penjaga is still open or has penjaga!"
            );
        }
    }
    ...
    private PenjagaModel passPenjaga(@PathVariable("noPenjaga") Long noPenjaga){
        try{
            return penjagaRestService.passingPenjaga(noPenjaga);
        }catch(NoSuchElementException e){
            throw new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "No Penjaga" + String.valueOf(noPenjaga) + " Not Found."
            );
